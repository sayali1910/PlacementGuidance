...
Builder.load_file('bestart/bestart.kv')
class Notify(ModalView):
    def __init__(self,**kwargs):
        super().__init__(**kwargs)
        self.size_hint=(.7,.7)
        
class BeStartWindow(BoxLayout):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        client=MongoClient()
        db=client.stud
    ...
    def write_post(self):
        ...
        if user_name.strip()== '' or company_name.strip()=='' or pckg.strip()=='' or post.strip()=='' or year.strip()=='' or selected.strip()=='':
            ...
            com=self.company.find_one({'UserName':user_name,'CompanyName':company_name,'Package':pckg,'Year':year})
            if com==None:
                self.company.insert_one({'UserName':user_name,'CompanyName':company_name,'Package':pckg,'Status':selected,'Year':year})
            else:
                self.notify.add_widget(Label(text='[color=#FF000][b]Data already present[/b][/color]',markup=True))
                self.notify.open()
                Clock.schedule_once(self.killswitch,1)
        ...
        #scrn_company_content.add_widget(comptabel)
    def get_Posts(self):
        client = MongoClient()
        db = client.stud
        posts = db.posts
    ...
    def change_screen(self,instance):
        if instance.text=='Post interview exp.':
